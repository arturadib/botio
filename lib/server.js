...
// runJob()
//@ + `event`: Event type (e.g. `cmd_test`, `push`, etc)
//@ + `issue`: Issue number (if event comes from issue comment or pull request)
...
function runJob(jobInfo, callback) {
  ...
  child.exec(commandLine, { 
    ...
    if (err) {
      log('Script failed ('+jobInfo.event+')');
      log('Cause:', err);
      if (jobInfo.issue) {
        postComment({
          issue: jobInfo.issue,
    ...
    log('Script successful ("'+jobInfo.event+'")');
    if (jobInfo.issue) {
      postComment({
        issue: jobInfo.issue,
        title: 'Success',
...
app.post('/', function(req, res) {
  ...
  case 'issue_comment':
    ...
    getPullDetails({issue: payload.issue.number}, function(err, pull) {
      ...
      enqueueJob(payload, pull, 'cmd_'+command);
    });

    //
    // Enqueue job
    //
    function enqueueJob(payload, pull, event) {
      var jobInfo = {
        event: event,
        issue: payload.issue.number,
        private_dir: path.resolve(config.private_dir+'/'+event+'-'+pull.head.short_sha),
        public_dir: path.resolve(config.public_dir+'/'+event+'-'+pull.head.short_sha),
        public_url: 'http://'+config.ip+':'+config.port+'/'+event+'-'+pull.head.short_sha,
      ...
      log('Command queued at #'+queue.size()+' ("'+event+'" from @'+payload.sender.login+')');
      postComment({
        issue: payload.issue.number,
        title: 'Received',
        body: 'Command **'+event+'** from @'+payload.sender.login+' added to queue. '+
    ...
    queue.push(function() {
      var event = 'push',
          jobInfo = {
            event: event,
            issue: null,
            private_dir: path.resolve(config.private_dir+'/'+event+'-'+payload.head_commit.id),
            public_dir: path.resolve(config.public_dir+'/'+event+'-'+payload.head_commit.id),
            public_url: 'http://'+config.ip+':'+config.port+'/'+event+'-'+payload.head_commit.id,
            base_url: null,
