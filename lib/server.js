...
if (config.github_secret) {
  app.use (function(req, res, next) {
    ...
    var hmac = crypto.createHmac('sha1', config.github_secret);
    req.setEncoding('utf8');
    req.on('data', function(chunk) {
      hmac.update(chunk);
    });
    req.on('end', function() {
      req.githubSha1 = hmac.digest('hex');
    });
    next();
  });
}
...
app.post('/', function(req, res) {
  ...
  var sha1 = req.header('x-hub-signature');
  if (config.github_secret) {
    debug('Server SHA1:', req.githubSha1);
    debug('Request SHA1:', sha1);
    if (('sha1=' + req.githubSha1) !== sha1) {
      log('Invalid request from github: SHA1 mismatch.');
      return;
    }
  }
  ...
  case 'issue_comment':
    ...
    var commentBody = payload.comment.body,
        handles = config.handles ? config.handles.join('|') : 'botio',
        match = commentBody.match(new RegExp('^\/(?:'+handles+') +(\\w+)', 'm'));
