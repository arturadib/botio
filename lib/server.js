  ...
  case 'issue_comment':
    ...
    function enqueueJob(payload, pull, command) {
      var jobInfo = {
        ...
        private_dir: path.resolve(config.private_dir+'/'+command+'-'+pull.head.sha),
        public_dir: path.resolve(config.public_dir+'/'+command+'-'+pull.head.sha),
        public_url: 'http://'+config.ip+':'+config.port+'/'+command+'-'+pull.head.sha,
      ...
      postComment({
        ...
        queue.push(function() {
          runJob(payload, pull, command, jobInfo);
        });
      });
    };
    ...
    // Run job
    //
    function runJob(payload, pull, command, jobInfo) {
